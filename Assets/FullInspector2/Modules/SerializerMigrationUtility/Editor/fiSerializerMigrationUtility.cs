...
namespace FullInspector.Modules.SerializerMigrationUtility {
    public static class fiSerializerMigrationUtility {
        ...
        public static void MigrateUnityObject(UnityObject obj, Type fromSerializer, Type toSerializer) {
            var restoreState = typeof(BehaviorSerializationHelpers).GetMethod("RestoreState").MakeGenericMethod(fromSerializer);
            var saveState = typeof(BehaviorSerializationHelpers).GetMethod("SaveState").MakeGenericMethod(toSerializer);
